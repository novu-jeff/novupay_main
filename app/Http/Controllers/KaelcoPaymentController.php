...
class KaelcoPaymentController extends Controller
{
    public function receiveEncrypted(Request $request)
    {
        Log::info("Incoming KAELCO Encrypted Payload", ['data' => $request->input('data')]);
        ...
        Log::info("Decrypted KAELCO Payload", $payload);
        ...
        if (!empty($missing) || !empty($invalid)) {
            Log::warning('KAELCO Missing or invalid required fields', [
                'missing' => $missing,
                'invalid' => $invalid,
        ...
        $surcharge = (float) $payload['surcharge'] ?? 0;
        ...
        if ($cType !== null && strtoupper(trim((string) $cType)) === 'R') {
            $novupay_fee = 10;
        }
        Log::info('Consumer Type', ['cType' => $cType, 'novupay_fee' => $novupay_fee]);

        // Select fee and default payment methods based on amount
        if ($amount < 800) {
        ...
        // Allow override from payload if ever needed
        if (!empty($payload['payment_methods']) && is_array($payload['payment_methods'])) {
            $paymentMethods = $payload['payment_methods'];
        }
        $paymentMethodsAll = ['gcash', 'qrph_netbank', 'upay_online'];

        // Calculate additional service fee including surcharge
        $hitpay_fee = round($selected_fee, 1);
        $additional_service_fee = $hitpay_fee + $novupay_fee;
        Log::info("Additional Service Fee", ['additional_service_fee' => $additional_service_fee, 'hitpay_fee' => $hitpay_fee, 'novupay_fee' => $novupay_fee, 'surcharge' => $surcharge]);
        $final_amount = round($total_amount + $additional_service_fee, 2);
        Log::info("Final Amount", ['final_amount' => $final_amount]);
        ...
        // Due date: use provided or fallback to +15 days
        try {
        ...
        $purpose = "Amount Due: ₱" . number_format($final_amount, 2) . "\n" .
                   "Convenience Fee: ₱" . number_format($additional_service_fee, 2) . "\n" .
                   "KAELCO Bill Payment for {$account_no}";

        // HitPay description format:
        // Amount Due
        // Convenience Fee
        // Account #
        ...
        if ($hitpayResponse->failed()) {
            Log::error('KAELCO HitPay API failed', ['response' => $hitpayResponse->body()]);
            return response()->json(['error' => 'Failed to contact HitPay'], 500);
        }

        $hitpayData = $hitpayResponse->json();
        Log::info('KAELCO HitPay Data', $hitpayData);
        ...
        $bill = KaelcoBill::create([
            ...
            'surcharge'        => $surcharge,
            'convenience_fee'  => $additional_service_fee,
        ...
        Log::info("Preparing to insert Payment Item", [
            ...
            'amount' => $disconnectionFee > 0 ? $disconnectionFee : (float) $payload['amount'],
        ]);
        ...
        Log::info("KAELCO Response Payload", $responsePayload);
